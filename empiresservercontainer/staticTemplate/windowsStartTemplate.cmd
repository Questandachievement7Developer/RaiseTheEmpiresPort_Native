...
::
:: Copyright 2017-2020 by Vegard IT GmbH (http://vegardit.com) and the cygwin-portable-installer contributors.
...
:: ABOUT
:: =====
:: This self-contained Windows batch file creates a portable Cygwin (http://cygwin.com/mirrors.html) installation.
:: By default it automatically installs :
:: - apt-cyg (cygwin command-line package manager, see http://github.com/kou1okada/apt-cyg)
:: - bash-funk (Bash toolbox and adaptive Bash prompt, see http://github.com/vegardit/bash-funk)
:: - ConEmu (multi-tabbed terminal, http://conemu.github.io/)
:: - Ansible (deployment automation tool, see http://github.com/ansible/ansible)
:: - AWS CLI (AWS cloud command line tool, see http://github.com/aws/aws-cli)
:: - testssl.sh (command line tool to check SSL/TLS configurations of servers, see http://testssl.sh/)
...
:: change the URL to the closest mirror http://cygwin.com/mirrors.html
...
:: if set to 'yes' the apt-cyg command line package manager (http://github.com/kou1okada/apt-cyg) will be installed automatically
set INSTALL_APT_CYG=yes

:: if set to 'yes' the bash-funk adaptive Bash prompt (http://github.com/vegardit/bash-funk) will be installed automatically
set INSTALL_BASH_FUNK=yes

:: if set to 'yes' Node.js (http://nodejs.org/) will be installed automatically
set INSTALL_NODEJS=yes
:: Use of the folder names found here http://nodejs.org/dist/ as version name.
set NODEJS_VERSION=latest-v12.x
:: one of: auto,64,32 - specifies if 32 or 64 bit version should be installed or automatically detected based on current OS architecture
set NODEJS_ARCH=auto

:: if set to 'yes' Ansible (http://github.com/ansible/ansible) will be installed automatically
set INSTALL_ANSIBLE=yes
set ANSIBLE_GIT_BRANCH=stable-2.9

:: if set to 'yes' AWS CLI (http://github.com/aws/aws-cli) will be installed automatically
set INSTALL_AWS_CLI=yes

:: if set to 'yes' testssl.sh (http://testssl.sh/) will be installed automatically
set INSTALL_TESTSSL_SH=yes
:: name of the GIT branch to install from, see http://github.com/drwetter/testssl.sh/
set TESTSSL_GIT_BRANCH=3.0

:: use ConEmu based tabbed terminal instead of Mintty based single window terminal, see http://conemu.github.io/
...
:: set Mintty options, see http://cdn.rawgit.com/mintty/mintty/master/docs/mintty.1.html#CONFIGURATION
...
:: http://blogs.msdn.microsoft.com/david.wang/2006/03/27/howto-detect-process-bitness/
if "%CYGWIN_ARCH%" == "auto" (
...
call :download "http://cygwin.org/%CYGWIN_SETUP_EXE%" "%CYGWIN_ROOT%\%CYGWIN_SETUP_EXE%"

:: Cygwin command line options: http://cygwin.com/faq/faq.html#faq.setup.cli
if "%PROXY_HOST%" == "" (
    set CYGWIN_PROXY=
) else (
...
(
    ...
    if "%INSTALL_CONEMU%" == "yes" (
        ...
        echo     echo "*******************************************************************************"
        echo     conemu_url="http://github.com$(wget http://github.com/Maximus5/ConEmu/releases/latest -O - 2>/dev/null | egrep '/.*/releases/download/.*/.*7z' -o)" ^&^& \
    ...
    if "%INSTALL_ANSIBLE%" == "yes" (
        ...
        echo     git clone http://github.com/ansible/ansible --branch %ANSIBLE_GIT_BRANCH% --single-branch --depth 1 --shallow-submodules /opt/ansible
        echo fi
        echo.
    )
    if "%INSTALL_AWS_CLI%" == "yes" (
    ...
    if "%INSTALL_APT_CYG%" == "yes" (
        ...
        echo     echo "*******************************************************************************"
        echo     wget -O /usr/local/bin/apt-cyg http://raw.githubusercontent.com/kou1okada/apt-cyg/master/apt-cyg
    ...
    if "%INSTALL_BASH_FUNK%" == "yes" (
        ...
        echo         git clone http://github.com/vegardit/bash-funk --branch master --single-branch --depth 1 --shallow-submodules /opt/bash-funk
        echo     elif hash svn ^&^>/dev/null; then
        echo         svn checkout http://github.com/vegardit/bash-funk/trunk /opt/bash-funk
        echo     else
        echo         mkdir /opt/bash-funk ^&^& \
        echo         cd /opt/bash-funk ^&^& \
        echo         wget -qO- --show-progress http://github.com/vegardit/bash-funk/tarball/master ^| tar -xzv --strip-components 1
        echo     fi
        echo fi
    )
    if "%INSTALL_NODEJS%" == "yes" (
        ...
        REM echo     nvm_version=$^(curl -s http://api.github.com/repos/nvm-sh/nvm/releases/latest ^| grep -Po 'tag_name": "\Kv[0-9.]+'^)
        REM echo     mkdir /opt/nvm
        REM echo     curl -o- http://raw.githubusercontent.com/nvm-sh/nvm/$nvm_version/install.sh ^| bash
        REM echo fi
        echo if [[ ! -e /opt/nodejs/current ]]; then
        echo     nodejs_ver=%NODEJS_VERSION%
        echo     if [[ $nodejs_ver == latest* ]]; then
        echo         nodejs_ver=$^(curl -s http://nodejs.org/dist/$nodejs_ver/ ^| grep -oP 'node-^(\K[v0-9.]+[0-9]^)' ^| head -n1^)
        echo     fi
        echo     node_js_root="/opt/nodejs/node-${nodejs_ver}-win-x%NODEJS_ARCH%"
        echo     echo "*******************************************************************************"
        echo     echo "* Installing [Node.js]..."
        echo     echo "*******************************************************************************"
        echo     curl http://nodejs.org/dist/${nodejs_ver}/node-${nodejs_ver}-win-x%NODEJS_ARCH%.zip -o nodejs.zip
    ...
    if "%INSTALL_TESTSSL_SH%" == "yes" (
        ...
        echo     [[ -e /opt ]] ^|^| mkdir /opt
        echo     if hash git ^&^>/dev/null; then
        echo         git clone http://github.com/drwetter/testssl.sh --branch %TESTSSL_GIT_BRANCH% --single-branch --depth 1 --shallow-submodules /opt/testssl
        echo     elif hash svn ^&^>/dev/null; then
        echo         svn checkout http://github.com/drwetter/testssl.sh/tags/%TESTSSL_GIT_BRANCH% /opt/testssl
        echo     else
        echo         mkdir /opt/testssl ^&^& \
        echo         cd /opt/testssl ^&^& \
        echo         wget -qO- --show-progress http://github.com/drwetter/testssl.sh/tarball/%TESTSSL_GIT_BRANCH% ^| tar -xzv --strip-components 1
        echo     fi
...
(
    ...
    echo     echo #    see http://cygwin.com/cygwin-ug-net/using.html#mount-table
    echo     echo.
    ...
    echo.
    :: http://stackoverflow.com/a/8452363/5116073
    echo set "arg1=%%~1"
    echo setlocal EnableDelayedExpansion
    echo if "!arg1!" == "" (
    if "%INSTALL_CONEMU%" == "yes" (
